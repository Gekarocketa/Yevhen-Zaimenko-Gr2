...
public class CalcBB {
	...
	public String calc() {
		if (kwota == null || okres == null || procent == null) {
			// пробуем явно прочитать POST параметры из запроса (учебный вариант)
			try {
	...
	// Демонстрация приема параметров методом GET
	...
	public String info() {
		// Демонстрация работы с HttpServletResponse через ExternalContext
		try {
			ExternalContext ec = ctx.getExternalContext();
